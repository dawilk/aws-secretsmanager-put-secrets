...
const SECRET_ARN = 'arn:aws:secretsmanager:us-east-1:123456789012:secret:my/secret-abc123';
...
describe('put-secrets action', () => {
    ...
    test('secret not found creates secret with CreateSecret', async () => {
        smMockClient
            ...
            .rejects(new ResourceNotFoundException({ $metadata: {}, message: 'Secret not found' }))
    ...
    test('CreateSecret fails with descriptive error', async () => {
        smMockClient
            ...
            .rejects(new InvalidParameterException({ $metadata: {}, message: 'Invalid name format' }));
    ...
    test('read fails throws clear error', async () => {
        smMockClient
            .on(GetSecretValueCommand)
            .resolves({ Name: SECRET_ID }); // No SecretString or SecretBinary - invalid response
    ...
    test('JSON identical logs up-to-date and sets workflow-run:check tag', async () => {
        ...
        expect(core.setFailed).not.toHaveBeenCalled();
        expect(core.info).toHaveBeenCalledWith(`Secret '${SECRET_ID}' is up-to-date.`);
        ...
        expect(tags).toContainEqual({ Key: 'github-actions:workflow-run:check', Value: 'https://github.com/owner/repo/actions/runs/12345' });
    });
    ...
    test('value changed puts secret and sets workflow-run:update tag', async () => {
        ...
        expect(tagCalls.length).toBeGreaterThan(0);
        const tags = tagCalls[0].args[0].input.Tags ?? [];
        expect(tags).toContainEqual({ Key: 'github-actions:workflow-run:update', Value: 'https://github.com/owner/repo/actions/runs/12345' });
    });

    test('tags merged and updated', async () => {
        mockGetInput({ 'tags': '{"Environment":"prod","Team":"platform"}', 'secret-value': 'new-value' });
