...
const TEST_ARN = 'arn:aws:secretsmanager:us-east-1:123456789012:secret:test/secret-abc123';
const SIMPLE_JSON = '{"api_key": "testkey", "user": "testuser"}';
...
describe('getSecretValue', () => {
    ...
    test('throws if unable to retrieve', async () => {
        smMockClient.on(GetSecretValueCommand).rejects(
    ...
    test('throws if secret value invalid (no name)', async () => {
        smMockClient.on(GetSecretValueCommand).resolves({ SecretString: TEST_VALUE });
        await expect(getSecretValue(smClient, TEST_NAME)).rejects.toThrow('Invalid name');
    });
});
...
describe('createSecret', () => {
    ...
    test('throws descriptive error on LimitExceededException', async () => {
        smMockClient.on(CreateSecretCommand).rejects(
            new LimitExceededException({ $metadata: {}, message: 'Quota exceeded' })
        );
    ...
    test('throws generic error for other CreateSecret failures', async () => {
        smMockClient.on(CreateSecretCommand).rejects(new Error('Internal server error'));
...
describe('isAuthError', () => {
    ...
    test('returns false for ResourceNotFoundException', () => {
        const err = new ResourceNotFoundException({ $metadata: {}, message: 'Not found' });
...
describe('jsonEqual', () => {
    ...
    test('returns true for equivalent JSON with different key order - all same key-values present', () => {
        ...
        const dbConfigA = '{"host":"db.example.com","port":5432,"database":"app","user":"admin","password":"secret"}';
        const dbConfigB = '{"password":"secret","user":"admin","database":"app","port":5432,"host":"db.example.com"}';
...
describe('valuesEquivalent', () => {
    ...
    test('real-world: database config JSON equivalent despite key order', () => {
        const existing = '{"host":"db.prod.com","port":3306,"database":"users","ssl":true}';
        const input = '{"database":"users","host":"db.prod.com","port":3306,"ssl":true}';
...
describe('parseTagsInput', () => {
    ...
    test('skips non-string non-number values', () => {
        const result = parseTagsInput('{"valid":"ok","nested":{"a":1},"arr":["x"]}');
...
describe('buildWorkflowRunUrl', () => {
    ...
    test('builds URL from env vars', () => {
        ...
        expect(buildWorkflowRunUrl()).toBe('https://github.com/owner/repo/actions/runs/12345');
    });
...
describe('mergeTags', () => {
    test('merges user tags with action tag', () => {
        const result = mergeTags(
            { Env: 'prod' },
            { key: 'github-actions:workflow-run:update', value: 'https://example.com/run/1' }
        );
...
describe('tagsNeedUpdate', () => {
    ...
    test('returns false when current has extra keys not in desired', () => {
        expect(tagsNeedUpdate({ Env: 'prod', Extra: 'x' }, { Env: 'prod' })).toBe(false);
    });
});
...
describe('putSecret orchestration', () => {
    ...
    test('creates secret when not found', async () => {
        smMockClient
            ...
            .rejects(new ResourceNotFoundException({ $metadata: {}, message: 'Not found' }))
    ...
    test('logs up-to-date when equivalent', async () => {
        ...
        expect(core.info).toHaveBeenCalledWith(`Secret '${TEST_NAME}' is up-to-date.`);
    ...
    test('real-world: JSON secret equivalent by key order does not trigger update', async () => {
        ...
        const dbConfigReordered = '{"database":"app","host":"db.example.com","port":5432}';
