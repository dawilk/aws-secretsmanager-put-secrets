...
const secretId = process.env.PUT_SECRET_TAGS_TEST_ID || 'PutSecretsIntegrationTest/TaggedSecret';
...
describe('Put secret tags integration', () => {
    it('secret has user tags and github-actions metadata tag', async () => {
        ...
        const tags = (response.Tags || []).reduce((acc, t) => {
        ...
        const hasActionTag =
            tags['github-actions:workflow-run:update'] || tags['github-actions:workflow-run:check'];
